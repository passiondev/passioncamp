...
class HelloSignController extends Controller
{
    public function __invoke(Request $request)
    {
        ...
        logger()->info($request->input('event.event_type'), ['signature_request_id' => $request->input('event.event_metadata.related_signature_id')]);
        ...
        switch ($request->input('event.event_type')) {
            ...
            case 'signature_request_all_signed':
                $waiver = Waiver::whereProvider('hellosign')
                    ->where('provider_agreement_id', $request->input('event.event_metadata.related_signature_id'))
                    ->firstOrFail();
